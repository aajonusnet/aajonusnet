...
$audioSyncAllowlist = $audioSyncAllowlist ?? [];
$audioSyncBlocklist = $audioSyncBlocklist ?? [];
...
function audioSyncEnabledFor(array $allowlist, array $blocklist, string $slug, string $path): bool
{
    $slugKey = strtolower($slug);
    $pathKey = strtolower(str_replace('\\', '/', $path));
    $normalize = fn($v) => strtolower(str_replace('\\', '/', $v));
    $allow = array_map($normalize, $allowlist);
    $block = array_map($normalize, $blocklist);

    if ($allow && !in_array($slugKey, $allow, true) && !in_array($pathKey, $allow, true)) {
        return false;
    }
    if ($block && (in_array($slugKey, $block, true) || in_array($pathKey, $block, true))) {
        return false;
    }
    return true;
}
...
<head>
    ...
    <?php if (!$is404): ?>
        <link rel="canonical" href="<?= $canonicalUrl ?>"><?php endif; ?>
    <link rel="stylesheet" href="<?= assetUrl('style.css') ?>?v=1">
    <link rel="icon" href="<?= assetUrl('logos/favicon.ico') ?>" type="image/x-icon" sizes="any">
    <link rel="apple-touch-icon" href="<?= assetUrl('logos/apple-touch-icon.png') ?>">
...
<body>
    <header>
        ...
        <div id="title-container">
            <a id="title" href="<?= assetUrl('') ?>">
                <h1><?= $originalFile ? $dynamicTitle : $logoTitle ?></h1>
            </a>
            <?php if ($originalFile) { ?>
                <a href="<?= assetUrl('') ?>" id="back-button" onclick="goBack(event)" aria-label="Go back" tabindex="0">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        <polyline points="13.42,5.41 4,12 13.41,18.59" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </a>
                <div id="share-button" onclick="shareArticle()" role="button" aria-label="Share" tabindex="0">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        <path d="M16 4l-1.42 1.42-1.59-1.59V15h-1.98V3.83L9.42 5.42 8 4l4-4 4 4zm4 5v11c0 1.1-.9 2-2 2H6c-1.11 0-2-.9-2-2V9c0-1.11.89-2 2-2h3v2H6v11h12V9h-3V7h3c1.1 0 2 .89 2 2z" fill="currentColor" />
                    </svg>
                </div>
            <?php } ?>
        </div>
    ...
    <?php if (!$originalFile && !$is404) { ?>
        ...
        <div id="search-container">
            <svg id="search-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 44">
                <g stroke="#757575" stroke-width="1.1" fill="none" stroke-linecap="butt">
                    <circle cx="6" cy="20" r="5" />
                    <line x1="10.039" y1="23.721" x2="13.909" y2="27.591" />
                </g>
            </svg>
        ...
        <nav id="categories" aria-label="Categories">
            <a href="<?= assetUrl('') ?>" onclick="filterCategory(event, 'All', 'All', this)">All</a>
            <?php
            ...
            foreach ($topLevelCategories as $category => $cat) {
                ...
                echo '<a href="' . assetUrl($category . '/') . '"' . $classAttr . ' onclick="filterCategory(event, \'' . $cat . '\', \'' . $category . '\', this)">' . $cat . '</a>';
            }
            ?>
        </nav>
        <main>
            <div id="grid">
                ...
                foreach ($articles as $article) {
                    ...
                    $dataId = htmlspecialchars(str_replace('\\', '/', $filePath), ENT_QUOTES, 'UTF-8');
                ...
                ?>
                    <div class="card-md" data-id="<?= $dataId ?>" <?= $hidden ?>>
                        ...
                        <h2><a class="read-more" href="<?= assetUrl($fullUrl) ?>"><?= $filename ?></a></h2>
                    </div>
                <?php } ?>
    ...
    <?php } else { ?>
        <div id="content"><?php
                            ...
                            if (!$file) {
                                echo '<p id="not-found">Page not found. Go to the <a href="' . assetUrl('') . '">homepage</a>.</p>';
                                echo '</div></body></html>';
                                exit;
                            }
                            ...
                            $audioSyncEnabled = strpos($content, '[audio]:') !== false
                                && audioSyncEnabledFor($audioSyncAllowlist, $audioSyncBlocklist, $slug, $originalFile);
                            $position = null;
                            ...
                            $content = preg_replace(
                                ['~^\[audio\]:\s*\(([^)]+)\)\s*$~m', '~^\[video\]:\s*\(([^)]+)\)\s*$~m'],
                                ['<audio controls preload="metadata" src="$1"></audio>', '<video controls playsinline preload="metadata" src="$1"></video>'],
                                $content
                            );

                            $imgBase = ($basePath !== '' ? $basePath : '') . '/imgs';
                            $content = preg_replace('/!\[\[(.*?) \| (\d+)\]\]/', '<img src="' . $imgBase . '/$1" alt="$1" width="$2">', $content);
                            $content = preg_replace('/!\[(.*?)\]\((.*?)\)/', '![$1](' . $imgBase . '/$2)', $content);
                            $content = preg_replace('/!\[\[(.*?)\]\]/', '![$1](' . $imgBase . '/$1 "$1")', $content);
                            ...
                            ?>
            <script>
                window.articlePath = <?= json_encode($originalFile ? str_replace('\\', '/', $originalFile) : null) ?>;
                window.audioSyncEnabled = <?= $audioSyncEnabled ? 'true' : 'false' ?>;
            </script>
        </div>
        <?= isset($_GET['s']) ? '<button id="remove-highlights"><span class="x">×</span>Highlights</button>' : '' ?>
        <script defer src="<?= assetUrl('code/findonpage.js') ?>"></script>
    <?php } ?>
    <script defer src="<?= assetUrl('code/index.js') ?>?v=1" id="main-js" data-cache="<?= $cacheHours ?>"></script>
    <script src="<?= assetUrl('code/audiosync.js') ?>"></script>
</body>
